...
const CreateBook = () => {
  ...
  const createBook = async (book: BookSchema) => {
    const newBook = { ...book, price: +book.price, stock: +book.stock };
    try {
      const { data } = await toast.promise(
        BookeraBackend.post('/book', newBook, {
  ...
  const uploadBookCover = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      createBookForm.setValue('cover', data.link);
    } catch (error) {
      return;
    } finally {
  ...
  const uploadBook = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      createBookForm.setValue('link', data.link);
    } catch (error) {
      return;
    } finally {
      setIsUploading(false);
  ...
  return (
    <form
      onSubmit={createBookForm.handleSubmit(createBook, (e) => console.log(e))}
    >
      <div className='grid gap-5'>
          ...
          return (
            <div className='grid gap-1' key={index}>
              ...
              <Input
                ...
                {...createBookForm.register(field.field)}
              />
              <small className='text-red-400 text-center block h-3'>
                {createBookForm?.formState?.errors[field.field]?.message}
              </small>
            </div>
          );
        })}
        <Select
          onValueChange={(value: Categories) =>
            createBookForm.setValue('category', value)
          }
        >
        ...
        <div className='grid w-full max-w-sm items-center gap-1.5'>
          ...
          />
          <small className='text-red-400 text-center block h-3'>
            {createBookForm?.formState?.errors['link']?.message}
          </small>
        </div>
        <div className='grid w-full max-w-sm items-center gap-1.5'>
          ...
          />
          <small className='text-red-400 text-center block h-3'>
            {createBookForm?.formState?.errors['cover']?.message}
          </small>
        </div>
      </div>
